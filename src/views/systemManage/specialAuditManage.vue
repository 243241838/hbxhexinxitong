    ...
    <div class="ggg_user">
          <div class="filter-container">
            <el-form :inline="true" :model="params" ref="params"  class="demo-form-inline">
                ...
                <el-form-item label="专题ID" prop="bizId">
                    <el-input v-model="params.bizId" placeholder="专题ID"></el-input>
                </el-form-item>
        ...
        <el-table :data="dataList" ref="table" v-loading="listLoading" border fit highlight-current-row :default-sort = "{prop: params.order, order: params.dir === 'desc' ? 'descending' : 'ascending'}" @sort-change="sort" >
            <el-table-column align="center" label="序号" width="100">
             ...
             <el-table-column align="center" label="专题策略ID">
                <template scope="scope">
            ...
            <el-table-column align="center" label="专题类型">
                <template scope="scope">
                    <span v-show="scope.row.specialType == 20">爆款</span>
                    <span v-show="scope.row.specialType != 20">普通</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="操作名称">
                <template scope="scope">
                    <span v-show="scope.row.itemType == 11">新增专题</span>
                    <span v-show="scope.row.itemType == 12">修改专题</span>
                </template>
            </el-table-column>
...
export default {
    data() {
        return {
            ...
            pickTimeEnd: '', //结束时间
            params: { 
                url: 'specialaudit/aduitSpecialListView',
                page: 1,
                size: 10,
                bizId: '', //专题id
                effTimestr: '', //开始时间
    ...
    methods: {
        ...
        getApplyList () {
            ...
            config(this.params).then(response => {
                ...
                if (response.respCode === '000000') {
                    ...
                    this.dataList = response.data.specialAuditRspList;
                } else {
        ...
        //去支付渠道列表
        go (val, number) {
            this.$router.push({ path: '/systemManage/auditDetail',query: { data: JSON.stringify(val)} })
        },
        //请求提示
        message (sts, text) {
            this.$message({
