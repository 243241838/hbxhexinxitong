                ...
                <el-col :span="20" :offset="1" >
                    <el-form  class="credit-aplly-form" ref="redForm" :model="redForm" :rules="rules" :inline="initInline">
                        ...
                        <el-row>
                            ...
                            <el-col :span="7">
                                <el-form-item>
                                    <uploadImg v-on:acqImgUrl="acqImgUrl" ref="img1" v-bind:paramName="'promotionLogo'"></uploadImg>
                                </el-form-item>
                            </el-col>
                        </el-row>
                                ...
                                <el-row>
                                    <table cellspacing="0" cellpadding="0" border="1" class="el-table__header" style="width: 960px;">
                                        ...
                                        <tr class="el-table__row">
                                            ...
                                            <td class="el-table_1_column_19">
                                                <div class="cell">{{item.offUpto==0?'无限制':('订单满'+item.offUpto/100+'可用')}}</div>
                                            </td>
                                        </tr>
                                    </table>
                                </el-row>
                                <el-row>
                                    ...
                                    <el-col :span="15">
                                        <el-form-item  :prop="'redPacketData['+topIndex+'].totalAmount'"
                                            ...
                                            <el-input v-model="item.totalAmount" :disabled="item.totalAmountDisabled" style="width:300px"></el-input>
                                        </el-form-item>
                                        <el-checkbox v-model="item.totalAmountNotLimitTmp" @change.native="checkIsLongTerm(topIndex,$event)">无限制</el-checkbox>
                                    </el-col>
                                </el-row>
                                <el-row>
                                    <el-col :span="4" align="center"  >
    ...
    export default {
        ...
        methods: {
            ...
            chooseRedPacket(redGiftInsuranceRow){
                ...
                redGiftInsuranceRow.forEach(function(v) {
                    if(!checkMap.has(v.couponId)){
                        ...
                        that.$set(that.output.redPacketData[that.output.redPacketData.length-1],'periodValue2','');
            ...
            commit () {
                ...
                this.output.redPacketData.forEach(function (vi) { //需要做一下处理，这两个字段，验证需要时间类型
                        vi.effDateStr = new Date(vi.effDateStr);
                        vi.expDateStr = new Date(vi.expDateStr);
                    }
                );
                ...
                this.redForm.url =  '/coupons/couponseditInfo';
            ...
            setInitData(initData,promotionId){
                this.redForm.promotionId =promotionId;
                this.redForm.promotionName=initData.promotionName;
                this.redForm.promotionIntro=initData.promotionIntro;
                this.redForm.promotionType=initData.promotionType;
                this.redForm.promotionChannels=initData.promotionChannels;

                if(initData.promotionChannels!='0'){//为0表示选中全部
                    this.promotionChannelsTmpArr= initData.promotionChannels.split(",");
                }


                this.redForm.promotionLogo=initData.promotionLogo;
                this.redForm.userType = initData.userType*1;
                this.redForm.effTimeStr=initData.effTimestr;
                this.redForm.expTimeStr=initData.expTimestr;
                this.redForm.productUseLimit = initData.productUseLimit;
                if(this.redForm.expTimeStr){
                    var str = this.redForm.expTimeStr.toString();
                    str = str.replace("/-/g", "/");
                    var expDate = new Date(str);
                    var nowDate = new Date();
                    if((expDate.getFullYear()-nowDate.getFullYear()) >900){ //系统
                        this.notExpTmp = true;
                    }
                }
                this.$refs.img1.setInitImg([{name: 'promotionLogo',
                    url: initData.promotionLogo}]);

                var that = this;
                initData.promotionCouponRecords.forEach(function (vmc) {
                    that.output.redPacketData.push({
                        couponId:vmc.couponId,
                        couponName:vmc.couponName,
                        offUpto:vmc.offUpto,
                        couponMinValue:vmc.couponMinValue,
                        couponMaxValue:vmc.couponMaxValue,
                        totalAmount:vmc.totalAmount==0?'':vmc.totalAmount,
                        totalAmountNotLimitTmp:vmc.totalAmount==0?true:false,
                        totalAmountDisabled:vmc.totalAmount==0?true:false,
                        userAmount:vmc.userAmount+'', //element校验 值需要时string 类型，可以加type,但是不知后台哪个返回是int
                        periodType:vmc.periodType==10?10:5,
                        effDateStr:null!=vmc.effDateStr?new Date(vmc.effDateStr):null,
                        expDateStr:null!=vmc.expDateStr?new Date(vmc.expDateStr):null,
                        periodValue:vmc.periodValue,
                        periodValue2:vmc.periodType!=10?(vmc.periodType):'',
                        promotionCouponName:vmc.promotionCouponName,
                        couponDesc:vmc.couponDesc

                    });
                });

                if(initData.productsList&&initData.productsList.length>0){
                    initData.productsList.forEach(function (vmc) {
                        that.productsResponsesData.push({
                            productId:vmc.productId,
                            productName:vmc.productName,
                            insurerName:vmc.insurerName,
                            productSource:vmc.productSource,
                            maxQuantity:vmc.maxQuantity,
                            totalQuantity:vmc.totalQuantity,
                            minPremium:vmc.minPremium,
                            maxPremium:vmc.maxPremium
                        });
                    });
                }

                if(initData.userList&&initData.userList.length>0){
                    initData.userList.forEach(function (vmc) {
                        that.userResponsesData.push({
                            userId:vmc.userId,
                            username:vmc.username,
                            userNickName:vmc.userNickName,
                            userLevel:vmc.userLevel,
                            userNickName:vmc.userNickName,
                            userGender:vmc.userGender
                        });
                    });
                }


                if(initData.productClassList&&initData.productClassList.length>0){
                    initData.productClassList.forEach(function (vmc) {
                        that.promotionProductTypeArr.push(vmc.classId);
                    });
                }
            },
