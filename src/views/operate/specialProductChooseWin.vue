<template>
    <div class="app-container calendar-list-container">
        ...
        <div class="filter-container">
            <span>审核状态:</span>
            <el-select v-model="listQuery.params.auditSts" placeholder="请选择" :disabled="auditStsDisabled">
        ...
        <div class="filter-container" v-if="selectType=='multiple'">
            <el-table :data="dataList" v-loading="listLoading" border fit
                ...
                <el-table-column align="center" prop="productName" label="来源类别">
                    <template scope="scope">
                        <span> {{scope.row.productSource==1?'自研':(scope.row.productSource==2?'保险公司':'其他')}}
                            </span>
                    </template>
...
<script>
    ...
    export default {
        data() {
            return {
                listLoading: true,
                listQuery: {params: {url: '/product/findSelectSpecialProduct', page: 1, size: 10,
                    auditSts: 1, bettemoney: '',productId:'',productName:'',
        ...
        props: {
            ...
            propAuditSts: {  // 父组件传过来的，决定组件是单选还是多选
                type: String
            },
        ...
        created() {
            ...
            //初始化 审核状态，一般产品的选择产品的状态都是必选的
            //如果页面没有传入 propAuditSts，那么默认审核状态为 审核通过，并且产品状态不可选
            //传入了，限制取消
            console.log(this.propAuditSts);
            if(this.propAuditSts!=undefined){
                this.listQuery.params.auditSts = this.propAuditSts;
                this.auditStsDisabled = false;
            }
        ...
        methods: {
            ...
            reset() {
                this.listQuery.params = {
                   url: '/product/findSelectSpecialProduct', page: 1, size: 10,
                    auditSts: 1, bettemoney: '',productId:'',productName:'',
                    classId:'',insurerId:'',maxProducePrice:'',minProducePrice:''
                };
            },
