                                ...
                                <el-row>
                                    ...
                                    <el-col :span="15">
                                        <el-form-item  :prop="'redGiftInsuranceData['+topIndex+'].userAmount'"
                                                       :rules="[{required: true, message: '个数不能为空', trigger: 'blur,change'}]">
    ...
    export default {
        ...
        methods: {
            ...
            chooseRedGiftInsurance(redGiftInsuranceRow){
                ...
                redGiftInsuranceRow.forEach(function(v) {
                    if(!checkMap.has(v.couponId)){
                        ...
                        that.$set(that.output.redGiftInsuranceData[that.output.redGiftInsuranceData.length-1],'userAmount','');
            ...
            commit () {
                ...
                this.output.redGiftInsuranceData.forEach(function (vi) { //需要做一下处理，这两个字段，验证需要时间类型
                ...
                if (!redFormFlag||!redGiftFormFlag) { //两个都通过了，可以入表
                    return ;
                }
            ...
            setInitData(initData,promotionId){
                ...
                initData.promotionCouponRecords.forEach(function (vmc) {
                    var startArray =  vmc.effTimestr.split("-");
                    var endArray = vmc.expTimestr.split("-");
                    var startDt = new Date(startArray[0], startArray[1]*1-1, startArray[2].substring(0,2));
                    var endDt = new Date(endArray[0], endArray[1]*1-1, endArray[2].substring(0,2));
                    ...
                    vmc.sts =str;
                    that.output.redGiftInsuranceData.push({
                        couponId:vmc.couponId,
                        couponName:vmc.couponName,
                        productLogo:vmc.productLogo,
                        effTimestr:vmc.effTimestr,
                        expTimestr:vmc.expTimestr,
                        sts:vmc.sts,
