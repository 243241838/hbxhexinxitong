        ...
        <div class="filter-container">
            <el-table :data="dataList" v-loading="listLoading" border fit highlight-current-row style="width: 100%" @current-change="handleCurrentChange">
                ...
                <el-table-column align="center" label="审核ID">
                    <template scope="scope">
                        <el-button @click="gocustomerDetail(scope.row.auditId,scope.row.bizId,scope.row.auditSts)" type="text" size="small">
                           {{scope.row.auditId}}
                        </el-button>
                    </template>
                </el-table-column>
...
export default {
    data() {
        return {
            ...
            pickTimeStart: '',
            pickTimeEnd: '',
            listQuery: { params: { url: 'audit/authAuditList', page: 1, size: 10, bizId: '',reqNickname: '',auditSts: '',effTime: '',expTime: ''} },
            current: 1, // 默认显示第一页
            total: 0,
            dataList: [],
    ...
    methods: {
        ...
        dateChange() {
            if (this.pickTimeStart === "") {
                this.listQuery.params.effTime = "";
            } else {
                this.listQuery.params.effTime = parseTime(this.pickTimeStart, '{y}-{m}-{d}');
            }
            if (this.pickTimeEnd === "") {
                this.listQuery.params.expTime = "";
            } else {
                this.listQuery.params.expTime = parseTime(this.pickTimeEnd, '{y}-{m}-{d}');
            }
        },
        //重置
        reset() {
            this.listQuery.params.bizId = "";
            this.pickTimeEnd = "";
            this.pickTimeStart = "";
            this.listQuery.params.reqNickname = "";
        ...
        //跳转进入详情页
        gocustomerDetail(val,val1, val2) {
            this.$router.push({ path: '/user/certificationAduitDetail',query: { auditId: val,bizId: val1, auditSts: val2} })
        }
    },
    filters: {
        //性别过滤
